...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-xl border p-5 shadow-lg transition-all",
  {
    variants: {
      variant: {
        default: "bg-pine border-sage/30 text-cream",
        destructive: "bg-red-600 border-red-800 text-white",
      },
    },
...
>(({ className, ...props }, ref) => (
  <ToastPrimitive.Action
    ref={ref}
    className={cn("inline-flex items-center justify-center rounded border border-sage/30 px-3 py-1 text-sm font-medium hover:bg-white/10 focus:outline-none", className)}
    {...props}
  />
));
...
>(({ className, ...props }, ref) => (
  <ToastPrimitive.Close
    ref={ref}
    className={cn("absolute right-2 top-2 rounded p-1 text-cream/50 hover:text-cream focus:outline-none", className)}
    toast-close=""
    {...props}
  >
    <X className="h-4 w-4" />
